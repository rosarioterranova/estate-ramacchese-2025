    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div ref={timelineRef} className="relative">
        {events.map((event, index) => {
          const isTargetEvent = event.isActive || (event.isToday && !events.some(e => e.isActive)) || (!event.isPast && !events.some(e => e.isActive || e.isToday));
          
          return (
            <div 
              key={`${event.date}-${event.time}-${index}`}
              ref={isTargetEvent && !activeEventRef.current ? activeEventRef : null}
            >
              <EventCard 
                event={event} 
